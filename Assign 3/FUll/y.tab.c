...
{
       0,    48,    48,    57,    62,    67,    72,    78,    85,   119,
     154,   160,   161,   163,   173,   186,   187,   190,   193,   199,
     205,   212,   220,   227,   233,   243,   244,   247,   248,   249,
     250,   251,   252,   253,   254,   262,   265,   266,   269,   275,
     292,   293,   331,   332,   477,   478,   693,   694,   757,   758,
     827,   850,   866,   869,   870,   903,   904,   905,   906,   933,
     967,   968,   970,   975
};
#endif
...
#line 154 "parser.y" /* yacc.c:1646  */
    {if((yyvsp[0])->name=="(") {table->EntryScope();}}
...
#line 163 "parser.y" /* yacc.c:1646  */
    {fprintf(logout,"Line no %d->parameter_list : parameter_list COMMA type_specifier ID\n %s \n\n",lin_count,(yyvsp[0])->name.c_str());
                 ...
                 else {table->InsertS((yyvsp[0])->name,(yyvsp[0])->type,-1111,-1111,1,0,-1111,-1111,type);
...
#line 220 "parser.y" /* yacc.c:1646  */
    {
                 fprintf(logout,"Line no %d->declaration_list : declaration_list COMMA ID LTHIRD CONST_INT RTHIRD\n %s\n\n",lin_count,(yyvsp[-3])->name);
                  ...
                  else {if(type==0) {table->InsertS((yyvsp[-3])->name,(yyvsp[-3])->type,-1111,-1111,(yyvsp[-1])->value.ival,0,-1111,-1111,2); table->AllScopePrint();}
                        else if(type==1){table->InsertS((yyvsp[-3])->name,(yyvsp[-3])->type,-1111,-1111,(yyvsp[-1])->value.ival,0,-1111,-1111,3); table->AllScopePrint();}                     
}
                    }
...
else if((yyvsp[-2])->value.c=0) fprintf(logout,"Value of %s is %f\n\n",(yyvsp[-2])->name.c_str(),(yyvsp[-2])->value.fval);
...
(yyval)=new SymbolInfo((yyvsp[-1])->name,(yyvsp[-1])->type,(yyvsp[-1])->value.ival,(yyvsp[-1])->value.fval,(yyvsp[-1])->value.arraysize,(yyvsp[-1])->value.arraypos,-1111,-1111,(yyvsp[-1])->value.c);
                          ...
                          if((yyvsp[-1])->value.c==0) {(yyvsp[-1])->value.ival=(yyvsp[-1])->value.ival+1; 
...
#line 970 "parser.y" /* yacc.c:1646  */
    {fprintf(logout,"Line no %d->arguments: arguments COMMA logic_expression\n\n",lin_count);               
                              if((yyvsp[-2])->value.c==0 || (yyvsp[-2])->value.c==1) {flow1[pos1]=(yyvsp[-2])->value.c; pos1++;}
...
#line 975 "parser.y" /* yacc.c:1646  */
    {fprintf(logout,"Line no %d->arguments: logic_expression\n\n",lin_count); 
               ...
               if((yyvsp[0])->value.c==0 || (yyvsp[0])->value.c==1) {flow1[pos1]=(yyvsp[0])->value.c; pos1++;}
               else {flow1[pos]=-2; pos++;}
}
