...
class SymbolInfo
{
        public:
               ...
               SymbolInfo(string name,string type,string symbol,int ival,float fval,int arraysize,int arraypos,int iarrayv,float farrayv,int c)
               {
                   this->name=name;
                   this->type=type;
                   this->symbol=symbol;
...
class ScopeTable
{
     ...
     public:
        ...
        void Insert(string name, string type,string symbol, int ival, float fval, int arraysize,int arraypos,int iarrayv, float farrayv, int c)
        {
            ...
            if (entry->next == NULL)
            {
                entry->next = new SymbolInfo(name,type,symbol,ival,fval,arraysize,arraypos,iarrayv,farrayv,c);
		...
		//fprintf(logout,"<%s,%s,%d",entry->getName(),entry->getType(),entry->value.c);
		       ...
		       //cout<<"<"<<entry->name<<","<<entry->type<<",";
                ...
                if(entry->para==1) { fprintf(logout,"{"); //cout<<"{";
                       ...
                       if(i!=entry->fsize-1) fprintf(logout,"%s:%d,",entry->variable[i].c_str(),entry->f[i]);
                       else fprintf(logout,"%s:%d",entry->variable[i].c_str(),entry->f[i]);
                }
                fprintf(logout,"} %s",entry->freturn.c_str());//cout<<"}";
                }
...
class SymbolTable
{
     public:
         ...
         void InsertS(string name,string type,string symbol,int ival,float fval,int arraysize,int arraypos,int iarrayv,float farrayv,int c)
         {
             stable->Insert(name,type,symbol,ival,fval,arraysize,arraypos,iarrayv,farrayv,c);
         }
